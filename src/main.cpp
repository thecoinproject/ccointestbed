...
#if defined(NDEBUG)
# error "Ccoin cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ccoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "ccoin Signed Message:\n";
...
{
    // Ccoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 1000 * uint64(100000000);
...
static const int64 nTargetTimespan =  1 * 2 * 60 * 60; // ccoin: 2 hours
static const int64 nTargetSpacing = 60; // ccoin: 1 minutes
static const int64 nInterval = nTargetTimespan / nTargetSpacing;

//add freicoin diff filter block to which hardfork will occur
//ccoin 2015 team
static const int64 nFilteredBlockChange = 309710; //testing for block 400k
static const int64 nFilteredInterval =    9; //9 blocks or 9 minutes
static const int64 nFilteredTargetTimespan = nFilteredInterval * nTargetSpacing; //  9 minutes
...
//ccoin 2015 team
    ...
    // Ccoin: This fixes an issue where a 51% attack can change difficulty at will.
    //first year of blocks were rough don't count them
    //updated by ccoin 2015 team
    int blockstogoback = nInterval-1;
    if ((pindexLast->nHeight+1) != nInterval)
        blockstogoback = nInterval;


    //check if we rae at new diff filter time yet
    if ((pindexLast->nHeight+1) < nFilteredBlockChange){
        //if not we will allow variance
        //limit adjustent step
	//limit adjustent step
    ...
    //check if we are at the diff filter time
    else{
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ccoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
	...
	//ignore block 15120 that has a bit of a time difference
	//eventually ignore it by hash, but sicne we have later checkpoints, juts ignore the blocknumber
//	if (nHeight == 15120) {
//           uint256 hashBlockOneFiveOneTwoZero = uint256("0x4c420c9cf2d529b5e5d36bc10a40d8300aa30764bdca5fa9983ede81a0738b68");
//	                    return state.DoS(100, error("AcceptBlock() : nHeight 15120 has hash agreed to"));
//        }
	//ignore blocks below 250000 (250k) sicne we have checkpoint past this block it should be okay
        if (nHeight > 309710 && (nBits != GetNextWorkRequired(pindexPrev, this)))
...
//no logner able to make easy assumption of dos
//        CBigNum bnNewBlock;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc3f245168555b26af66905e63dc382b40ed054ed0cf3a25930389405f01d2ed3");
    }
    //
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1389382112;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2801637;

        if (fTestNet)
        {
            block.nTime    = 1388998431;
            block.nNonce   = 1702690;
        }
...
unsigned char pchMessageStart[4] = { 0xfc, 0xd9, 0xb7, 0xdd }; //ccoin
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CcoinMiner(CWallet *pwallet)
{
    printf("CcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ccoin-miner");
    ...
    try { loop {
        ...
        printf("Running CcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CcoinMiner, pwallet));
}

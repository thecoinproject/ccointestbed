...
//allow first year of blocks time intervals to be long or short from slow and fast hashing
static const int64 nGoBackTo = 300000; //blocks with long changes over last year of no updates ccoin 2015 team
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    //check if we are past the first year of blocks, if so implement retarget again
    if ((pindexLast->nHeight+1) < nGoBackTo){
    ...
    //check if we are past the first year of blocks
    else{
        int64 nActualTimespan = pindexLast->GetBlockTime() - pindexFirst->GetBlockTime();
        printf("  nActualTimespan = %"PRI64d"  before bounds\n", nActualTimespan);
        if (nActualTimespan < nTargetTimespan/4)
...
//        }
	//ignore blocks below 309480 (309k) sicne we have checkpoint at this block it should be okay
        if (nHeight > 309480 && (nBits != GetNextWorkRequired(pindexPrev, this)))

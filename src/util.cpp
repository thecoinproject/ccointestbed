...
string FormatMoney(const mpq &q, bool fPlus)
{
    ...
    const mpq q_abs = abs(q) / mpq(1000000/1);
...
bool ParseMoney(const char* pszIn, int64& nRet)
{
    ...
    if (strWhole.size() > 10) // guard against 63 bit overflow
        return false;
    if (nUnits < 0 || nUnits > uint64(100000000))
        return false;
    int64 nWhole = atoi64(strWhole);
    int64 nValue = nWhole*uint64(100000000) + nUnits;
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ccoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ccoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ccoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Ccoin will not work properly.");
